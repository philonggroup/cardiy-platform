...
export default function HomePage() {
    ...
    const [reminderPlate, setReminderPlate] = useState('');
    const [reminderKm, setReminderKm] = useState('');
    const [reminderService, setReminderService] = useState('Bao duong dinh ky');
    const [partnerName, setPartnerName] = useState('');
    const [partnerPhone, setPartnerPhone] = useState('');
    const [partnerProvince, setPartnerProvince] = useState('');
    const [partnerType, setPartnerType] = useState('Xe luu dong');
  ...
  const submitReminder = (e: React.FormEvent) => {
        e.preventDefault();
        setReminderOk(true);
        setTimeout(() => { setReminderOk(false); setShowReminder(false); }, 3000);
  };

  const submitPartner = (e: React.FormEvent) => {
        e.preventDefault();
        setPartnerOk(true);
        setTimeout(() => { setPartnerOk(false); setShowPartner(false); }, 3000);
  };

  const activeProvinces = provinces.filter(function(p) { return p.active; });
    const shownProvinces = activeTab === 'all' ? provinces : activeProvinces;
    const totalXe = provinces.reduce(function(a, b) { return a + b.count; }, 0);
  ...
  return (
        <div className="min-h-screen bg-[#0F172A] pb-24">
              ...
              <div className="px-4 py-6">
                      ...
                      <p className="text-gray-400 text-sm mb-5">CRM thong minh danh cho chuoi dich vu o to. Quan ly dat lich, KTV, bang gia, xuat bao cao Excel.</p>p>
                      <div className="space-y-3">
                                <Link href="/dashboard" className="flex items-center gap-3 w-full bg-[#F59E0B] text-black rounded-2xl px-5 py-4 font-semibold text-base hover:bg-yellow-400 transition-all active:scale-95">
              ...
              <div className="px-4 pb-6">
                      ...
                      <div className="grid grid-cols-4 gap-3">
                        {services.map((s, i) => (
                      <Link key={i} href={s.href} className="flex flex-col items-center gap-1 bg-[#1E293B] rounded-2xl p-3 border border-[#334155] hover:border-yellow-500 transition-all active:scale-95">
              ...
              <div className="px-4 pb-6">
                      <div className="bg-gradient-to-r from-[#1E293B] to-[#162032] rounded-3xl p-5 border border-[#334155]">
                                ...
                                <button
                                              ...
                                              className="w-full bg-[#F59E0B] text-black rounded-2xl py-3 font-bold text-base hover:bg-yellow-400 transition-all active:scale-95"
                                            >
              ...
              <div className="px-4 pb-6">
                      <div className="bg-[#1E293B] rounded-3xl p-5 border border-[#334155]">
                                ...
                                <p className="text-gray-400 text-sm mb-4">Phu song toan quoc · {activeProvinces.length} tinh thanh dang hoat dong</p>p>
                      
                                <div className="flex gap-2 mb-4">
                                            <button
                                                            onClick={() => setActiveTab('all')}
                                                            className={activeTab === 'all' ? 'px-4 py-2 rounded-xl text-sm font-medium bg-yellow-500 text-black' : 'px-4 py-2 rounded-xl text-sm font-medium bg-[#0F172A] text-gray-400 border border-[#334155]'}
                                                          >
                                                          Tat ca ({provinces.length})
                                            </button>button>
                                            <button
                                                            onClick={() => setActiveTab('active')}
                                                            className={activeTab === 'active' ? 'px-4 py-2 rounded-xl text-sm font-medium bg-green-500 text-black' : 'px-4 py-2 rounded-xl text-sm font-medium bg-[#0F172A] text-gray-400 border border-[#334155]'}
                                                          >
                                                          Dang HD ({activeProvinces.length})
                                            </button>button>
                                </div>div>
                      
                                <div className="grid grid-cols-2 gap-2 mb-4">
                                  {shownProvinces.map((p, i) => (
                        <div key={i} className={p.active ? 'flex items-center justify-between bg-[#0F172A] rounded-xl px-3 py-2 border border-green-500/30' : 'flex items-center justify-between bg-[#0F172A] rounded-xl px-3 py-2 border border-[#334155]'}>
                                ...
                                <div className="bg-[#0F172A] rounded-2xl p-4 border border-[#334155] mb-4">
                                            <div className="flex justify-between items-center mb-2">
                                                          ...
                                                          <span className="text-white font-bold">{totalXe} xe</span>span>
                                            </div>div>
                                            <div className="flex justify-between items-center mb-2">
                                                          <span className="text-gray-400 text-sm">Tinh thanh phu song</span>span>
                                                          <span className="text-white font-bold">{provinces.length}/63 tinh</span>span>
                                            </div>div>
                                ...
                                <button
                                              ...
                                              className="w-full bg-blue-600 text-white rounded-2xl py-3 font-bold text-base hover:bg-blue-500 transition-all active:scale-95"
                                            >
              ...
              <div className="px-4 pb-6">
                      <div className="bg-[#1E293B] rounded-3xl p-5 border border-[#334155]">
                                ...
                                <div className="grid grid-cols-3 gap-2 mb-5">
                                  {partners.map((p, i) => (
                        <div key={i} className="bg-[#0F172A] rounded-xl p-3 text-center border border-[#334155] hover:border-yellow-500 transition-all">
                                ...
                                <button
                                              onClick={() => setShowPartner(true)}
                                              className="w-full bg-yellow-500 text-black rounded-2xl py-3 font-bold text-base hover:bg-yellow-400 transition-all active:scale-95"
                                            >
                  ...
                  <div className="fixed inset-0 bg-black/70 z-50 flex items-end justify-center p-4">
                            <div className="bg-[#1E293B] rounded-3xl p-6 w-full max-w-md border border-[#334155]">
                                        <div className="flex items-center justify-between mb-4">
                                                      ...
                                                      <button onClick={() => setShowReminder(false)} className="text-gray-400 text-2xl hover:text-white">x</button>button>
                                        </div>div>
                              {reminderOk ? (
                                  <div className="text-center py-8">
                                                  <span className="text-5xl">✅</span>span>
                                                  <p className="text-white font-bold text-lg mt-3">Dang ky thanh cong!</p>p>
                                                  <p className="text-gray-400 text-sm mt-1">Chung toi se nhac ban dung han</p>p>
                                  </div>div>
                                ) : (
                                  <form onSubmit={submitReminder} className="space-y-3">
                                                  <div>
                                                                    ...
                                                                    <input
                                                                                          ...
                                                                                          className="w-full bg-[#0F172A] text-white rounded-xl px-4 py-3 border border-[#334155] outline-none focus:border-yellow-500"
                                                  ...
                                                  <div>
                                                                    ...
                                                                    <input
                                                                                          type="text"
                                                                                          value={reminderPlate}
                                                                                          onChange={e => setReminderPlate(e.target.value)}
                                                                                          className="w-full bg-[#0F172A] text-white rounded-xl px-4 py-3 border border-[#334155] outline-none focus:border-yellow-500 uppercase"
                                                  ...
                                                  <div>
                                                                    ...
                                                                    <select
                                                                                          ...
                                                                                          className="w-full bg-[#0F172A] text-white rounded-xl px-4 py-3 border border-[#334155] outline-none"
                                                                                        >
                                                                                        <option>Bao duong dinh ky</option>option>
                                                                                        <option>Thay dau nhot</option>option>
                                                                                        <option>Thay lop xe</option>option>
                                                                                        <option>Kiem tra phanh</option>option>
                                                                                        <option>Dang kiem xe</option>option>
                                                                                        <option>Gia han bao hiem</option>option>
                                                                    </select>select>
                                                  </div>div>
                                                  <button type="submit" className="w-full bg-yellow-500 text-black rounded-2xl py-3 font-bold text-base hover:bg-yellow-400 transition-all mt-2">
                                                                    Xac nhan dang ky
                                                  </button>button>
                                  </form>form>
                                        )}
                  ...
                  <div className="fixed inset-0 bg-black/70 z-50 flex items-end justify-center p-4">
                            <div className="bg-[#1E293B] rounded-3xl p-6 w-full max-w-md border border-[#334155]">
                                        <div className="flex items-center justify-between mb-4">
                                                      ...
                                                      <button onClick={() => setShowPartner(false)} className="text-gray-400 text-2xl hover:text-white">x</button>button>
                                        </div>div>
                              {partnerOk ? (
                                  <div className="text-center py-8">
                                                  <span className="text-5xl">🎉</span>span>
                                                  <p className="text-white font-bold text-lg mt-3">Dang ky thanh cong!</p>p>
                                                  <p className="text-gray-400 text-sm mt-1">Doi ngu CARDIY se lien he trong 24h</p>p>
                                  </div>div>
                                ) : (
                                  <form onSubmit={submitPartner} className="space-y-3">
                                                  <div>
                                                                    <label className="text-gray-400 text-sm block mb-1">Ten / Ten garage *</label>label>
                                                                    <input
                                                                                          type="text"
                                                                                          value={partnerName}
                                                                                          onChange={e => setPartnerName(e.target.value)}
                                                                                          className="w-full bg-[#0F172A] text-white rounded-xl px-4 py-3 border border-[#334155] outline-none focus:border-yellow-500"
                                                                                          placeholder="Garage Minh Tuan"
                                                                                          required
                                                                                        />
                                                  </div>div>
                                                  <div>
                                                  ...
                                                  <div>
                                                                    <label className="text-gray-400 text-sm block mb-1">Tinh / Thanh pho *</label>label>
                                                                    <select
                                                                      ...
                                                                      {provinces.map((p, i) => (
                                                                                                                <option key={i} value={p.name}>{p.name}</option>option>
                                                                                                              ))}
                                                  ...
                                                  <div>
                                                                    ...
                                                                    <select
                                                                                        ...
                                                                                        <option>Nha cung cap phu tung</option>option>
                                                                    </select>select>
                                                  </div>div>
                                                  <button type="submit" className="w-full bg-yellow-500 text-black rounded-2xl py-3 font-bold text-base hover:bg-yellow-400 transition-all mt-2">
                                                                    🚀 Dang ky ngay
